...
def data_access(i_time, P_level, date_Ymd, extend = None, get_thickness = False):
    """Load one analysis time from the ERA5 archive and return a plotting-ready dictionary."""
    ...
    # ERA5 latitude is stored north-to-south, so reverse it to match the plotting arrays below.
    ...
    # Convert geopotential (m^2 s^-2) to approximate geopotential height in dam.
    ...
    # Scale vorticity into 10^-5 s^-1 so the contour labels stay in a meteorology-friendly range.
    ...
    if get_thickness:
        ...
        R = 287
        g = 9.89
        # Hypsometric thickness between adjacent pressure levels, then summed from 1000 to 500 hPa.
    ...
    # Convert accumulated precipitation rate from meters to inches for the U.S.-style plot labels.
    ...
    if not(extend is None):
        ...
        # Subset every field to the requested plotting window after the latitude reversal above.
        lon = lon[index_lon]
        lat = lat[index_lat]
